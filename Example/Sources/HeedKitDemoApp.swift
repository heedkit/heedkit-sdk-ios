...
// MARK: - CONFIG (edit me)
//
// Point the demo at your local Rails backend and a workspace key.
//
//   1. Run the backend:   cd heedkit-rails && bin/dev   (serves :3000)
//   2. Grab a workspace key from the console Install page (or db/seeds —
//      the seeded "heedkit"/"demo" workspace). Never commit a real key.
//   3. Paste it into `workspaceKey` below, OR pass it at launch with the
//      HEEDKIT_WORKSPACE_KEY / HEEDKIT_API_URL environment variables
//      (Xcode → Scheme → Run → Arguments → Environment Variables).
//
enum Config {
    /// Public workspace key (safe to ship in client code). Placeholder by default —
    ...
    static var keyIsPlaceholder: Bool { workspaceKey == "pk_REPLACE_ME" }
